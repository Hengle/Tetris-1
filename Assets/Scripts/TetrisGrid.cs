...
public class TetrisGrid
{
    ...
    bool MoveActiveGroup(Coordinate to) {
        foreach (var member in activeGroup) {
            foreach (var brick in member) {
                if (!MoveActiveBrick(brick, to + brick.coordinate()))
                    return false;
            }
        }
        return true;
    ...
    //Moves the active group from the Grid, so the correct 
    //placements might be initiated
    ...
    bool IsLineFilled(int ind) {
        if (ind >= 0 && ind < Grid.GetLength(1)) {
            for (int i = 0; i < Grid.GetLength(0); i++) {
                if (!Grid[i, ind].isSome || activeGroup.Contains(Grid[i, ind]))
                    return false;
            }
            return true;
        }
    ...
    void DropLineAbove(int row) {
        for (var collumnIndex = 0; collumnIndex < Grid.GetLength(0); collumnIndex++) {
            for (var rowIndex = row + 1; rowIndex < Grid.GetLength(1); rowIndex++) {
                MoveNormalBrick(new Coordinate(collumnIndex, rowIndex), 
                    new Coordinate(collumnIndex, rowIndex - 1));
            }
        }
    }
